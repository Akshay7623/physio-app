...
const sendOTP = async (req, res) => {
    ...
    const { mobile, name, password, old_user } = req.body;
        ...
        if (response.data.status === 200) {
            ...
            axios(opt).then(async (resp) => {
                ...
                if (resp.data.responseCode === 200) {
                    ...
                    if (old_user === 1) {
                        const new_user = await UserModel.updateOne({ mobile: mobile }, { first_name: name, created_at: created_at, otp_id: resp.data.data.verificationId, otp_token: response.data.token });
                        res.json({ message: 'success' });
                    } else {
                        const new_user = UserModel({ mobile: mobile, first_name: name, created_at: created_at, verified: 0, otp_id: resp.data.data.verificationId, otp_token: response.data.token });
